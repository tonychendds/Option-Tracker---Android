...
The Positions list shows a delayed last price for each underlying stock, such as `NVDA  $178.42`, with a space before the Open badge. Tickers are requested once per unique symbol from Yahoo Finance’s public spark feed (`query1.finance.yahoo.com`). No API key is required. Prices are cached for about 90 seconds, and pull-to-refresh loads them again. The list footer says quotes are delayed 15+ minutes. If the phone is offline or the request fails, the position stays and the stock price is an em dash (—).

The same row keeps the entry premium and adds a delayed quote for that contract, such as `entry $1.80 · now $1.42`. The contract is the OCC symbol: root, expiration, call or put, and strike. `BRK.B` becomes `BRKB`. When bid and ask are both present the quote is their midpoint; otherwise it is the last price. Contracts are requested once. If that quote fails, the line stays `entry $1.80 · now —`.

When that current premium is known, the right side of the same line shows unrealized P/L. A short position is `(entry − current) × contracts × 100`. A long position is `(current − entry) × contracts × 100`. Fees are not included. Positive amounts are green and negative amounts are red, for example `+$540.00` or `-$23,898.00`. If the option quote is missing, that amount is left off the row.

Next to the stock price, a red **ITM** chip appears only when the delayed stock price is in the money. ATM and OTM show no moneyness chip. ATM is still the band within 0.5% of the strike, including an exact match. A call is in the money above that band. A put is in the money below it. If the stock quote is missing, the chip is omitted. The days-to-expiration chip stays beside it.
