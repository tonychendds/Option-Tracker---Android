...
object Money {
    ...
    private val roundedAmountPattern = Regex("""\d+(\.\d+)?""")
    ...
    /**
     * Dollars rounded half-up to the nearest cent.
     * Spreadsheet cells often have more than two decimal places (`11.0`, `0.0000`, `7.2727`).
     */
    fun parseRoundedCents(raw: String): Long? {
        val cleaned = raw.trim()
            .replace("$", "")
            .replace(",", "")
            .replace(" ", "")
        if (!roundedAmountPattern.matches(cleaned)) return null
        return try {
    ...
    /** Signed dollars rounded half-up to the nearest cent. */
    fun parseSignedRoundedCents(raw: String): Long? {
        var cleaned = raw.trim()
            .replace("$", "")
            .replace(",", "")
            .replace(" ", "")
        ...
        val cents = parseRoundedCents(cleaned) ?: return null
        return if (negative) -cents else cents
    }
